...
describe 'rt4_service_test::apache_postgresql' do
  cached(:chef_run) do
  ...
  context 'compiling the test recipe' do
    ...
    before do
      stub_command('/usr/bin/psql -l | grep rt4default').and_return(false)
    end
  ...
  context 'stepping into rt4_service[rt4-default] resource' do
    ...
    # APACHE
    it 'installs installs apache' do
      expect(chef_run).to create_httpd_service('rt4-default')
    end
    it 'installs apache config' do
      expect(chef_run).to create_httpd_config('rt4-default')
    end
    it 'installs mod_fcgid' do
      expect(chef_run).to create_httpd_module('fcgid')
    end
    it 'do not starts/enables fcgi service' do
      expect(chef_run).to_not enable_service('rt4-default-fcgi')
      expect(chef_run).to_not start_service('rt4-default-fcgi')
    end

    # POSTGRESQL
    it 'installs DBD::pg from cpan' do
      expect(chef_run).to create_cpan_module('DBD::Pg')
    end
    it 'includes postgresql::server recipe' do
      expect(chef_run).to include_recipe('postgresql::server')
    end
    it 'does add postgres user to web group' do
      expect(chef_run).to modify_group('www-data')
    end
    %w(schema.Pg acl.Pg initialdata).each do |f|
      it 'sets postgres rt4 init files to be owned by postgres user' do
        expect(chef_run).to create_file("/opt/rt4-default/etc/#{f}").with(
          user: 'postgres',
          group: 'www-data'
        )
      end
